...
/**
 * The Class DeleteDrugPurposeCommand.
 * 
 * @author Artem Piven
 * @version 1.0
 */
public class DeleteDrugPurposeCommand
	...
	private static final Logger logger = LogManager
			.getLogger(DeleteDrugPurposeCommand.class);

	@Override
	public Router execute(RequestData data) {
		Router pageData = new Router();
		pageData.setPage(returnCurrentPage(data));
		if (data.getParameterValue(RequestParameterName.DRUG_ID) != null) {
			long drugId = Long
					.valueOf(data.getParameterValue(RequestParameterName.DRUG_ID)[0]);
			PurposeService service = PurposeServiceImpl.getInstance();
			try {
				boolean isDeleted = service.deleteDrugPurpose(drugId);
				if (isDeleted) {
